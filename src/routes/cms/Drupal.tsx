...
import { auditDrupal, type AuditResult, type Finding, type Severity } from '../../lib/drupalAudit';
import {
  toMarkdown,
  toHtml,
  downloadFile,
...
const STORAGE_KEY = 'pwn:drupalaudit:v1';
...
export default function Drupal() {
  ...
  const run = async () => {
    ...
    try {
      const r = await auditDrupal(WORKER_URL, state.target.trim(), (l) => setProgress(l));
      setResult(r);
      pushAudit({
        target: r.target,
  ...
  // exportMarkdown viejo eliminado — el módulo drupalExport ahora maneja
  ...
  return (
    <div className="space-y-10">
      <header className="space-y-4">
        <Prompt cwd="~/cms-audit" command={`./drupal-audit ${state.target || '<target>'}`} />
        <div className="flex items-start gap-4">
          ...
          <div>
            <h1 className="text-3xl font-semibold text-fg font-mono tracking-tight">Drupal Audit</h1>
            <p className="mt-2 max-w-2xl text-sm text-fg-muted leading-relaxed font-mono">
              Auditoría pasiva serverless: analizamos la seguridad expuesta de tu sitio Drupal (archivos de configuración, CHANGELOG, instaladores activos, y leaks del core) de forma ética.
            </p>
          </div>
        </div>
      </header>
      ...
      {/* Control Panel */}
      <Terminal title="drupal-scanner --dashboard">
      ...
      {loading && (
        <Terminal title="drupal-audit --status=active" className="border-accent/40 shadow-glow">
      ...
      {error && (
        <Terminal title="drupal-scanner --error" className="border-accent-red/40 shadow-glowRed">
      ...
      {result && (
        <div className="space-y-8 animate-fadeIn">
          {/* Header Summary */}
          <Terminal title="drupal-scanner --resumen" className="border-borderCustom">
            <div className="grid gap-8 md:grid-cols-[auto_1fr] items-center">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 font-mono">
                <div className="space-y-3">
                  ...
                  <MetaRow k="Drupal Core Version" v={result.metadata.drupalVersion || 'No detectada'} accent={!!result.metadata.drupalVersion} />
          ...
          <section className="space-y-4">
            ...
            {result.findings.length === 0 ? (
              <Terminal title="drupal --findings=clear" className="border-accent-green/30">
                ...
                {result.findings.map((f) => {
                  ...
                  return (
                    <FindingItem
                      ...
                      drupalVersion={result.metadata.drupalVersion}
                    />
                  );
                })}
              </div>
...
function FindingItem({
  ...
  const epUrl = cmsEndpointUrl(finding, target, 'drupal');
  const refs: SecRefs | null = getSecRefs(finding, 'drupal');
  const showCveLink = !!drupalVersion && isVersionRelated(finding);
    ...
    >
      ...
      {hasRefs && (
        <div className="flex flex-wrap items-center gap-1.5 px-4 pb-3 -mt-1">
          ...
          {showCveLink && drupalVersion && (
            <RefBadge
              href={cveSearchUrl('drupal', drupalVersion)}
              label={`Buscar CVEs · Drupal ${drupalVersion}`}
              title={`Búsqueda en NVD para Drupal ${drupalVersion}`}
              accent
            />
          )}
        </div>
  ...
  return (
    <div className="rounded-xl border border-borderCustom bg-bgSurface/50 p-4 space-y-3">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <button
          type="button"
          onClick={() => downloadFile(html, `drupal_${fname}.html`, 'text/html;charset=utf-8')}
        ...
        <button
          type="button"
          onClick={() => downloadFile(md, `drupal_${fname}.md`, 'text/markdown;charset=utf-8')}
        ...
        <button
          type="button"
          onClick={() =>
            downloadFile(jsonStr, `drupal_${fname}.json`, 'application/json;charset=utf-8')
          }
          className={EXPORT_SECONDARY}
        >
