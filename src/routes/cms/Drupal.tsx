...
export default function Drupal() {
  ...
  const exportMarkdown = () => {
    if (!result) return;
    let md = `# Reporte de Auditoría Drupal - ${result.target}\n\n`;
    md += `- **Fecha:** ${new Date(result.startedAt).toLocaleString()}\n`;
    md += `- **Score de Seguridad:** ${result.score}/100 (${result.riskLabel})\n`;
    md += `- **Versión Drupal:** ${result.metadata.drupalVersion || 'No detectada'}\n`;
    ...
    link.href = url;
    link.setAttribute('download', `drupal_audit_${result.target.replace(/https?:\/\//, '').replace(/[^a-zA-Z0-9]/g, '_')}.md`);
